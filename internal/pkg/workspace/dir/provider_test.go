...
func TestProvider_Provision_WhenSourcePathNotAbsolute_ThenReturnsErrSourceNotAbsolute(t *testing.T) {
	fs := afero.NewMemMapFs()
	provider := NewProvider(fs, "/runs")
	uri := url.URL{Scheme: "dir", Path: "relative/path"}

	_, err := provider.Provision(context.Background(), uri)

	require.ErrorIs(t, err, ErrSourceNotAbsolute)
}

func TestProvider_Provision_WhenSourceNotFound_ThenReturnsErrSourceNotFound(t *testing.T) {
	fs := afero.NewMemMapFs()
	provider := NewProvider(fs, "/runs")
	uri := url.URL{Scheme: "dir", Path: "/nonexistent"}
...
func TestProvider_Provision_WhenSourceNotDirectory_ThenReturnsErrSourceNotDirectory(t *testing.T) {
	fs := afero.NewMemMapFs()
	require.NoError(t, afero.WriteFile(fs, "/src/file.txt", []byte("data"), 0644))
	provider := NewProvider(fs, "/runs")
	uri := url.URL{Scheme: "dir", Path: "/src/file.txt"}
...
func TestProvider_Provision_WhenStatFails_ThenReturnsError(t *testing.T) {
	...
	provider := NewProvider(fs, "/runs")
	uri := url.URL{Scheme: "dir", Path: "/src"}
...
func TestProvider_Provision_WhenSuccess_ThenCopiesFilesAndReturnsWorkDir(t *testing.T) {
	...
	result, err := provider.Provision(context.Background(), uri)
