...
func TestNewInstance_WhenContextCanceled_ThenReturnsContextError(t *testing.T) {
	...
	_, err := newInstance(ctx, agent.ExecutionID("exec-1"), agent.ExecutionInput{}, RuntimeConfig{}, agent.RuntimeFeatures{}, t.TempDir())
...
func TestNewInstance_WhenWorkingDirectoryProvided_ThenUsesProvidedDir(t *testing.T) {
	...
	instance, err := newInstance(context.Background(), agent.ExecutionID("exec-1"), input, RuntimeConfig{}, agent.RuntimeFeatures{}, t.TempDir())
...
func TestInstance_Events_ReturnsEventsChannel(t *testing.T) {
	resetGeminiMockEnv(t)
	setGeminiMockExecutable(t)
	instance, err := newInstance(context.Background(), agent.ExecutionID("exec-1"), agent.ExecutionInput{Prompt: "Hello"}, RuntimeConfig{}, agent.RuntimeFeatures{}, t.TempDir())
	require.NoError(t, err)
